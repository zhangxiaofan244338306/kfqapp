...
define([
	...
	function(declare, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented, template,  lang, query,
		html, array, Deferred, esriLang, QueryTask, FeatureSet, PopupTemplate,esriRequest, PopupRenderer,
		...
		return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
			...
			_createQueryResultItem: function(options) {
				...
				var _titleName = popupTemplate2.info.title;
				var titleName = _titleName.substring(1, _titleName.length - 1);
				var titleValue = feature.attributes[titleName]
				...
				var strItem = '<tr class="jimu-table-row jimu-table-row-separator query-result-item" ' +
					...
					'<tbody >' +
					'<tr>' +
					'<td class="symbol-td"></td>' + titleValue +
			...
			_newExport: function() {
				...
				for(var i = 0; i < featureSet.features.length; i++) {
					objectIds += featureSet.features[i].attributes.OBJECTID;
					objectIds += ','
				}
				objectIds = objectIds.substring(0, objectIds.length - 1);
				
				//this._export(objectIds).then(console.log("export success!"));
				window.location.href="http://zj081:8015/ExportExcel.ashx?OBJIDS="+objectIds;
			},
			
//			_export: function(objectIds) {
//				return esriRequest({
//					url: 'http://localhost/kfqserver/ExportExcel.ashx?OBJIDS=7,8,9',
//					handleAs: "json"
//				});
//			},
			...
			_onBtnMenuClicked: function(evt) {
				...
				this.featureActionManager.getSupportedActions(featureSet, layer).then(lang.hitch(this, function(actions) {
					...
					var exportExcelAction = new BaseFeatureAction({
						name: "exportExcel",
						iconClass: 'icon-export',
						label: '导出到Excel',
						iconFormat: 'svg',
						map: this.map,
						onExecute: lang.hitch(this, this._newExport)
					});
					exportExcelAction.name = "exportExcel";
					exportExcelAction.data = featureSet;
					actions.push(exportExcelAction);
