...
define([
    ...
    function(declare, lang, array, html, query, Color, on, dom, has, _WidgetBase, _TemplatedMixin,
             _WidgetsInTemplateMixin, TooltipDialog, dojoPopup, Evented, template,FeatureLayer, FeatureTable, Chart, DefaultAxis,
             InvisibleAxis, Columns, Bars, Lines, Pie, ClusteredColumns, ClusteredBars, Tooltip, Highlight,
             MoveSlice, Magnify, SimpleTheme, symbolJsonUtils, Graphic, InfoTemplate, Point, SimpleLineSymbol, SimpleFillSymbol,PictureMarkerSymbol,
             graphicsUtils, esriLang,FeatureTable, jimuUtils,
             portalUrlUtils, Parameters) {
        ...
        return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
            ...
            _createCategoryModeColumnChart: function(args, chartDiv, data, paramsDijit){
                ...
                array.forEach(data, lang.hitch(this, function(item, index){
                    ...
                    for(var i = 0; i < item.valueFields.length; i++){
                        ...
                        seriesArray[i].push({
                            y: num,
                            tooltip: "<div class='jimu-widget-chart-preview-tooltip'" +
                            "style='color:" + this.tooltipColor + ";'>" +
                            "<table style='border-collapse:separate; border-spacing:8px;'><tbody><tr>" +
                            "<td><div style='white-space:nowrap'>" + a + "</div></td>" +
                            "<td>:</td>" +
                            "<td><div style='white-space:nowrap'>" + text + "</div></td>" +
                            "</tr><tr>" +
                            "<td><div style='white-space:nowrap'>" + aliasName + "</div></td>" +
                            "<td>:</td>" +
                            "<td><div style='white-space:nowrap'>" + c + "</div></td>" +
                            "</tr></tbody></table>" +
                            "</div>"
                        });
                    }
                ...
                //if isBigPreview is true, we need to enlarge chartDiv's size to let it can show all labels
                //this may result in scrollbar
                if(this.isBigPreview && labels.length > 0){
                    var minWidth = 16.5 * labels.length + 100;
                    if(chartDiv.clientWidth < minWidth){
                        chartDiv.style.width = minWidth + "px";
                    }
                }
            ...
            _createCategoryModeBarChart: function(args, chartDiv, data, paramsDijit){
                ...
                //this may result in scrollbar
                if(this.isBigPreview && labels.length > 0){
                    var minHeight = 16.5 * labels.length;
                    if(chartDiv.clientHeight < minHeight){
                        chartDiv.style.height = minHeight + "px";
                    }
                }
                ...
                array.forEach(data, lang.hitch(this, function(item, index){
                    ...
                    for(var i = 0; i < item.valueFields.length; i++){
                        ...
                        seriesArray[i].push({
                            ...
                            "<td><div style='white-space:nowrap'>" + c + "</div></td>" +
                            "</tr><tr>" +
                            "<td><div style='white-space:nowrap'>" + percent + "</div></td>" +
                            "</tr></tbody></table>" +
                            "</div>"
                        });
                    }
            ...
            _createCountModeColumnChart: function(args, chartDiv, data, paramsDijit){
                ...
                array.forEach(data, lang.hitch(this, function(item, index){
                    ...
                    series.push({
                        ...
                        "style='color:" + this.tooltipColor + ";'>" +
                        "<table style='border-collapse:separate; border-spacing:8px;'><tbody><tr>" +
                        "<td><div style='white-space:nowrap'>" + categoryAlias + "</div></td>" +
                        "<td>:</td>" +
                        "<td><div style='white-space:nowrap'>" + text + "</div></td>" +
                        "</tr><tr>" +
                        "<td><div style='white-space:nowrap'>" + this.nls.count + "</div></td>" +
                        "<td>:</td>" +
                        "<td><div style='white-space:nowrap'>" + b + "</div></td>" +
                        "</tr></tbody></table>" +
                        "</div>"
                    });
                }));
